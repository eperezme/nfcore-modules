process DORADO_CORRECT {
    tag "$meta.id"
    label 'process_high'

    conda "${moduleDir}/environment.yml"
    container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
        'https://depot.galaxyproject.org/singularity/YOUR-TOOL-HERE':
        'docker.io/eperezme/dorado:nano' }"

    input:
    tuple val(meta), path("*.fastq")

    output:
    tuple val(meta), path("*_corrected_reads.fasta.gz"), emit: bam
    path "versions.yml"           , emit: versions

    when:
    task.ext.when == null || task.ext.when

    script:
    def args = task.ext.args ?: ''
    def prefix = task.ext.prefix ?: "${meta.id}"

    // If kit is specified, pass it to dorado
    def additional_args = ""
    if (params.index_size) {
        additional_args += " --index-size $params.index_size"
    }
    if (params.batch_size) {
        additional_args += " --batch-size $params.batch_size"
    }

    """
    // Run the dorado basecaller with the specified mode and arguments
    dorado correct $input $args $additional_args > ${prefix}_corrected_reads.fasta && gzip ${prefix}_corrected_reads.fasta

    // Create a versions.yml file with the dorado version information
    cat <<-END_VERSIONS > versions.yml
    "${task.process}":
        dorado: \$(echo \$(dorado --version 2>&1) | sed -r 's/.{81}//')
    END_VERSIONS
    """
}
